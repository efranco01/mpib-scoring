...
class Util:
    ...
    def findDirectory(self, testname=str):
        for dir in os.listdir(os.getcwd()):
            if testname in dir and os.path.isdir(os.path.join(os.getcwd(), dir)):
        ...
        log.info(f'No directory named {testname} found in current directory, checking for files')
        return None
    
    def findInput(self, visit=list, testID=str):
        # If not visit is specified, run default (defaults to 1st visit)
        if visit == []:
    ...
    def askVisit(self):
        ...
        visit = input('Please enter the visit number(s) you would like to score (1, 2 or both): ')
        visitList = visit.split()
        # Check viability of input, check if the visit number is 1, 2 or both (separated by spaces or commas)
        if visit == '':
            print('No visit number entered. Please enter a valid visit number.')
            self.askVisit()
        elif visit.isdigit() == False:
            print('Invalid visit number. Please enter a valid visit number.')
            self.askVisit()
        else:
            return visitList
    ...
    def runFull(self): # rerun program for selected ID's
        while True:
    ...
    def runPart(self): # rerun program without inputs (for second visit)
        # This method should not ask further input from the user, 
        # instead the input has been given in the first instance
        
        # This run should include the findDirectory method
